...
yaml_file = open('CNNOpenCloseModel.yaml', 'r')
loaded_model_yaml = yaml_file.read()
yaml_file.close()
cnn = model_from_yaml(loaded_model_yaml)
# load weights into new model
cnn.load_weights("CNNOpenCloseWeights.h5")
...
def checkSleeping():
    ...
    while( end  < 15):
        ...
        for face in faces:
            ...
            left=None
            for i in cnn.predict_classes(xlefteye):
                left=i
            print(left)
            right=None
            for i in cnn.predict_classes(xrighteye):
                right=i
            print(right)
...
def playSound():
    song = AudioSegment.from_mp3("Alarm Sound.mp3")
    play(song)
